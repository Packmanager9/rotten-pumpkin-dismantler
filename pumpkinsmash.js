...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class LineOP {
        ...
        ohypotenuse() {
            let xdif = this.object.x - this.target.x
            let ydif = this.object.y - this.target.y
            let hypotenuse = (xdif * xdif * 2) + (ydif * ydif)
    ...
    class SpringOP {
        ...
        balance() {
            ...
            let xmomentumaverage = (this.body.xmom + this.anchor.xmom) / 2
            let ymomentumaverage = (this.body.ymom + this.anchor.ymom) / 2
            this.body.xmom = ((this.body.xmom * this.smoothrat) + xmomentumaverage) / (1 + this.smoothrat)
            this.body.ymom = ((this.body.ymom * this.smoothrat) + ymomentumaverage) / (1 + this.smoothrat)
            this.anchor.xmom = ((this.anchor.xmom * this.smoothrat) + xmomentumaverage) / (1 + this.smoothrat)
            this.anchor.ymom = ((this.anchor.ymom * this.smoothrat) + ymomentumaverage) / (1 + this.smoothrat)
        }
        draw() {
            this.beam.draw()
        }
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            // ball.xmom += (ball.origin.x - ball.x) * .1
            // ball.ymom += (ball.origin.y - ball.y) * .1
        ...
        function continued_stimuli(e) {
            ...
            XS_engine = e.clientX - FLEX_engine.left;
            YS_engine = e.clientY - FLEX_engine.top;
            X_diff = XS_engine - TIP_engine.x
            Y_diff = YS_engine - TIP_engine.y
            TIP_engine.x = XS_engine
            TIP_engine.y = YS_engine
            TIP_engine.body = TIP_engine
    ...
    function getRandomPumpkinColor() { // random color
        var letters = '0123456789ABCDEF';
        var color = '#';
        for (var i = 0; i < 2; i++) {
            color += letters[(Math.floor(Math.random() * 2) + 13)];
        }
        for (var i = 0; i < 2; i++) {
            color += letters[(Math.floor(Math.random() * 5) + 7)];
        }
        color += "00"
        return color;
    }
    ...
    class Pumpkin {
        constructor(y) {
            this.bits = []
            this.struck = 0
            this.center = new Circle(90 + (Math.random() * 1100), y, 100, "orange")
            this.springs = []
            this.pumpkincolor = getRandomPumpkinColor()
            let j = 0
            for (let t = 0; t < 90; 1 + 1) {
                j++
                if (j > 10000000) {
                    break
                }
                let point = new Circle(Math.random() * 1280, Math.random() * 720, 10, this.pumpkincolor)
                let link = new LineOP(point, this.center)
                if (link.ohypotenuse() < this.center.radius) {
                    t++
                    ...
                    point.friction = .96
                    point.reflect = 1
                    this.bits.push(point)
                }
            }
            for (let t = 0; t < this.bits.length; t++) {
                for (let k = 0; k < this.bits.length; k++) {
                    if (t != k) {
                        if (this.bits[t].touched.includes[k] || this.bits[k].touched.includes[t]) {
                            continue
                        }
                        let link = new LineOP(this.bits[t], this.bits[k])
                        if (link.hypotenuse() < 45) {
                            let spring = new SpringOP(this.bits[t], this.bits[k], link.hypotenuse() + 5, 12, this.pumpkincolor)
                            this.springs.push(spring)
                            this.bits[t].touched.push(k)
                            this.bits[k].touched.push(t)
                        }
                    }
                }
            }
            ...
            for (let t = 0; t < this.bits.length; t++) {
                for (let k = 0; k < this.bits.length; k++) {
                    if (t != k) {
                        if (this.bits[t].touched2.includes[k] || this.bits[k].touched2.includes[t]) {
                            continue
                        }
                        let link = new LineOP(this.bits[t], this.bits[k])
                        if (link.hypotenuse() < 55) {
                            let spring = new SpringOP(this.bits[t], this.bits[k], link.hypotenuse() + 5, 5 + (Math.random() * 6), "yellow")
                            this.springs2.push(spring)
                            this.bits[t].touched2.push(k)
                            this.bits[k].touched2.push(t)
                        }
                    }
                }
            }
        ...
        draw() {
            ...
            for (let t = 0; t < this.bits.length; t++) {
                if(this.bits[t].y > 800){
                    this.bits.splice(t,1)
                    t--
                }
            }
            ...
            for (let t = 0; t < this.bits.length; t++) {
                if (this.struck == 0) {
                    this.bits[t].xmom -= (this.bits[t].x - this.bits[t].origin.x) / 10
                    this.bits[t].ymom -= (this.bits[t].y - this.bits[t].origin.y) / 10
                    if (this.bits[t].origin.y < 720) {
                        this.bits[t].origin.y += 1.2
                    } else {
                        this.struck = 1
                    }
                } else {
                ...
                if (ball.fired == 1) {
                    ball.radius = ball.bigradsto
                    if (this.bits[t].doesPerimeterTouch(ball)) {
                        if(time > 0){
                            combo++ 
                        }
                        this.struck = 1
                        let link = new LineOP(ball, this.bits[t])
                        this.bits[t].xmom = -Math.cos(link.angle()) * link.hypotenuse() * 1
                        this.bits[t].xmom += ball.xmom
                        this.bits[t].ymom = -Math.sin(link.angle()) * link.hypotenuse() * 1
                        this.bits[t].ymom += ball.ymom
                        this.bits[t].x -= Math.cos(link.angle()) * link.hypotenuse() * .5
                        this.bits[t].y -= Math.sin(link.angle()) * link.hypotenuse() * .5
                    }
                }
                this.bits[t].frictiveMove()
            }

            canvas_context.beginPath()
            for (let t = 0; t < this.springs2.length; t++) {
                if (this.springs2[t].beam.hypotenuse() > 100) {
                    this.springs2[t].marked = 1
                }
                this.springs2[t].draw()
            }
            ...
            canvas_context.beginPath()
            for (let t = 0; t < this.springs.length; t++) {
                if (this.springs[t].beam.hypotenuse() > 50) {
                    this.springs[t].marked = 1
                }
                this.springs[t].draw()
            }
            canvas_context.lineWidth = 12
            canvas_context.strokeStyle =  this.pumpkincolor 
            canvas_context.stroke()
            for (let t = 0; t < this.springs.length; t++) {
                if (this.springs[t].marked > 0) {
                    this.springs.splice(t, 1)
                }
            }
            for (let t = 0; t < this.springs2.length; t++) {
                if (this.springs2[t].marked > 0) {
                    this.springs2.splice(t, 1)
                }
            }
        }
    }
    ...
    let pumpkin = new Pumpkin(360)
    let pumpkins = [pumpkin]
    let ball = new Circle(640, 360, 10, "white", 0, 0)
    ball.radsto = 10
    ball.bigradsto = 20
    ball.origin = new Circle(640, 360, 20, "white", 0, 0)
    ball.fired = 0
    ball.sxmom = 0
    ball.symom = 0
    ...
    let time = 999999
    let offset = (60000*2)
    let megagrid = new Image()
    megagrid.src = "megagrid.png"
    let pumpkinsmash = new Audio()
    pumpkinsmash.src = "pumpkinsmash.mp3"

    function main() {
        pumpkinsmash.play()
        canvas_context.clearRect(0, 0, canvas.width, canvas.height)  // refreshes the image
        canvas_context.drawImage(megagrid,0,0)
        time = (diff-Date.now())+offset
        gamepadAPI.update() //checks for button presses/stick movement on the connected controller)
        // // game code goes here
        for (let t = 0; t < pumpkins.length; t++) {
            pumpkins[t].draw()
        }
        // if (ball.fired == 1) {
        //     ball.move()
        // } else {
        //     let link = new LineOP(ball.origin, ball, "red", 2)
        //     ball.sxmom += (ball.origin.x - ball.x) * .1
        //     ball.symom += (ball.origin.y - ball.y) * .1
        //     ball.sxmom += (TIP_engine.x - ball.x) * .9
        //     ball.symom += (TIP_engine.y - ball.y) * .9
        //     ball.sxmom *= .5
        //     ball.symom *= .5
        //     ball.x += ball.sxmom
        //     ball.y += ball.symom
        ...
        ball.radius = ball.radsto
        ball.draw()

        let link = new Line(ball.origin.x, ball.origin.y, ball.x, ball.y, "red", 2)
        if (link.hypotenuse() > 700) {

            ball = new Circle(640, 360, 20, "white", 0, 0)
            ball.radsto = 10
            ball.bigradsto = 20
            ball.origin = new Circle(640, 360, 20, "white", 0, 0)
            ball.fired = 0
        ...
        counter++
        if (counter % 100 == 0) {
            let pumpkin = new Pumpkin(100)
            pumpkins.push(pumpkin)
        }


        for (let t = 0; t < pumpkins.length; t++) {
        if(pumpkins[t].bits.length <= 1){
            pumpkins.splice(t, 1)
        }
    }
    canvas_context.fillStyle = "white"
    canvas_context.font = "30px arial"
    canvas_context.fillText(combo, 10,33)
    canvas_context.fillText(Math.max(Math.floor(time/1000),0), 10,63)
    }
})
